...
void showProgressIndicator(BuildContext context) {
  showDialog(
    ...
    builder: (BuildContext context) {
      return Center(
        child: Column(mainAxisSize: MainAxisSize.min, children: const [Center(child: CircularProgressIndicator())]),
      );
    },
  );
}
...
class PayWithPaymentMethod extends StatelessWidget {
  const PayWithPaymentMethod({Key? key}) : super(key: key);
  ...
  Future<String?> prepareOrder() async {
    ...
    if (savedCustomerId != null) {
      ...
      customer = {"email": "test@example.com", "contact_number": "01010101010", "first_name": "Smith", "last_name": "Doe"};
    }
  ...
  Future<dynamic> inaiPayWithPaymentMethod(BuildContext context, String orderId, String paymentMethodId) async {
    try {
      InaiConfig config =
          InaiConfig(token: Constants.token, orderId: orderId, countryCode: Constants.country);

      InaiCheckout checkout = InaiCheckout(config);

      final result = await checkout.presentPayWithPaymentMethod(
          paymentMethodId: paymentMethodId, context: context);
  ...
  void payWithPaymentMethod(BuildContext context) async {
    // Obtain shared preferences.
    SharedPreferences prefs = await SharedPreferences.getInstance();

    //  Do we have a saved payment method id?
    ...
    if (savedPaymentMethodId != null) {
      ...
      InaiResult result = await inaiPayWithPaymentMethod(context, orderId!, savedPaymentMethodId);
      ...
      showAlert(context, "Payment Method Id Not Available", title: "", callback: () {
        navigateBackToHome(context);
      });
    }
  }
